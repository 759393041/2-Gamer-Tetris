    ...
    , ui(new Ui::MainWindow)
{
    ...
    m_client->OpenNet("192.168.1.5",8000);

    //开启子窗口服务


   connect(m_client,SIGNAL(SIG_ReadyData(uint,char*,int)),this->ow,SLOT(slot_ClientDealData(uint ,char*,int)));
...
void MainWindow::paintEvent(QPaintEvent *)
{
    ...
    BoardTrasInfo c_to_s;
    c_to_s.mark=MAIN_BOARD_TRASFORM_MARK;
    ...
    for(int n = 0; n < NUM_OF_BOARDS; n++)
    {
        ...
        for(int r = 0; r < h; r++)
        {
            for(int c = 0; c < w; c++)
            {
                ...
                if(n==MAIN_BOARD&&acell.get_color()!=NO_COLOR)
                {
                    c_to_s.opponents_colorful_cell.push_back(acell);
    ...
    char*buf_c_to_s=(char*)&c_to_s;

    m_client->SendData(0,buf_c_to_s,sizeof(c_to_s));
