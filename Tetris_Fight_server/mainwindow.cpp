    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(m_server,SIGNAL(SIG_ReadyData(uint,char*,int)),this->ow,SLOT(slot_ServerDealData(uint ,char*,int)));
    connect(m_server,SIGNAL(SIG_ReadyData(uint,char*,int)),this,SLOT(slot_mainwindowserver_dealdata(uint ,char*,int)));


    client_socket=0;

}
void MainWindow::slot_mainwindowserver_dealdata(uint socket ,char*,int)
{
    ...
    for(int n = 0; n < NUM_OF_BOARDS; n++)
    {
        ...
        for(int r = 0; r < h; r++)
        {
            for(int c = 0; c < w; c++)
            {
                ...
                if(n==MAIN_BOARD)
                {
                    S_to_C.opponents_colorful_cell.copy(acell);
                    S_to_C.mark=MAIN_BOARD_TRASFORM_MARK;
                    if(client_socket!=0)
                    {
                            char*buf_c_to_s=(char*)&S_to_C;
                            m_server->SendData(client_socket,buf_c_to_s,sizeof(S_to_C));
                    }
